...
type RowPredicate<TRow> = (row: TRow) => boolean;
type ValuePredicate = (rawValue: unknown) => boolean;

/**
 * Compile the row-independent half of one simple condition once. In
 * particular, right-hand operands are coerced once and LIKE patterns become
 * one reusable RegExp rather than being rebuilt for every row.
 */
const compileOperator = (
  ...
  if (operator === "IS NULL") return (rawValue) => isNullish(rawValue);
  ...
  const likeRegex =
    ...
    typeof right === "string"
      ? likeToRegExp(right, false)
      : (operator === "ILIKE" || operator === "NOT ILIKE") &&
          typeof right === "string"
        ? likeToRegExp(right, true)
        : undefined;
  ...
  return (rawValue) => {
    ...
    switch (operator) {
      ...
      case "LIKE":
      case "ILIKE":
        return likeRegex?.test(String(value)) ?? false;
      case "NOT LIKE":
      case "NOT ILIKE":
        return !(likeRegex?.test(String(value)) ?? false);
...
/**
 * Compile a `Condition` tree into a row predicate. The condition and column
 * filter types are fixed for one listing plan, so only row-dependent value
 * access/coercion remains on the per-row path.
 */
export function compileCondition<TRow>(
  ...
  const column = condition.left;
  const matches = compileOperator(
    condition.operator,
    condition.right,
    getFilterType?.(column)
  );
  return (row) => matches(getValue(row, column));
}

/** Evaluate one `Condition` against one row. Listing execution should prefer
 *  `compileCondition` so row-independent work is not repeated. */
