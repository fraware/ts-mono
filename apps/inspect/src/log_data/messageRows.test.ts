...
describe("inMemoryMessageRows", () => {
  ...
  it("resolves message and folded tool-message ids to row positions", async () => {
    const source = inMemoryMessageRows(messages);
    const all = await source.getRows(forward(undefined, 100));
    const assistantRow = all.rows.findIndex(
      (r) => r.resolved.message.id === "a-1"
    );

    await expect(source.resolveMessage("a-1")).resolves.toBe(assistantRow);
    // tool results fold into their parent assistant row
    await expect(source.resolveMessage("t-2")).resolves.toBe(assistantRow);
    await expect(source.resolveMessage("nope")).resolves.toBeUndefined();
  });
