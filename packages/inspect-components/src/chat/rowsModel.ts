...
/**
 * Row-window view-model for the chat list: exact folded-row total, loaded-or-
 * not random access, and a hook for requesting rows the viewport needs. A
 * model is immutable — producers hand out a new instance when loaded rows
 * change.
 */
export interface MessageRowsModel {
  total: number;
  /** The row at `index`, or undefined while it isn't loaded. */
  rowAt: (index: number) => MessageRow | undefined;
  /** Ask for rows `[start, end)` to be loaded (idempotent, cheap). */
...
/** A fully-resident model over already-built rows. */
export const messageRowsModel = (rows: MessageRow[]): MessageRowsModel => ({
  total: rows.length,
